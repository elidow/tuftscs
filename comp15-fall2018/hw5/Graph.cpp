...
//       (in case the edge did not exist) 
 ...
 void Graph<Vertex>::add_edge(Vertex f, Vertex t, string movie)
 {
     ...
     if (edges[row][col].compare(NULL_EDGE) == 0)
         edges[row][col] = movie;
 }
...
void Graph<Vertex>::report_path(ostream &out, Vertex start, Vertex end)// const
{
    ...
    if((s.get_size() - 1) == 0)
    {
        out << start << " and " << end << ": No connection";
    }
    else
    {
    ...
    while (!s.is_empty())
    {
        ...
        if(s.is_empty())
            break;
        Vertex v2 = s.top();
        out << v << " was in " << get_edge(v, v2) << " with ";
        out << v2 << endl;
    }
}

template<class Vertex>
string Graph<Vertex>::get_edge(Vertex f, Vertex t)
{
    int row;
    int col;
