...
class UCSAlgorithm:
    ...
    # simulates UCS algorithm
    def UCS(self, pancake_stack):

        # initialize stack and priority queue
        stack = Stack(pancake_stack)
        priority_q = PriorityQueue()
        priority_q.put((0, stack, []))
        ...
        while True:
            ...
            for i in range(SIZE - 1):
                ...
                if len(past_flips) == 0 or i != past_flips[len(past_flips) - 1]:
                    ...
                    new_total_cost = 1 + len(past_flips)
...
def main():
    ...
    while pancake_option != 3:
        ...
        # intro and menu
        print("\nWelcome to the Pancake Problem (with UCS)!\n")
        ...
        if pancake_option == 1 or pancake_option == 2:
            ...
            # run ucs algorithm
            agent = UCSAlgorithm()
            results = agent.UCS(initial_stack)
